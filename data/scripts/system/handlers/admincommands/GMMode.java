  ...
  public void execute(Player admin, String... params) {
    ...
    if (params[0].toLowerCase().equals("on")) {
      ...
      if (!admin.isGmMode()) {
        ...
        PacketSendUtility.sendPacket(
          ...
          new SM_MOTION(
            admin.getObjectId(),
            admin.getMotions().getActiveMotions()
          )
        );
        admin.updateKnownlist();
        PacketSendUtility.sendMessage(
          admin,
          "you are now Available and Wispable by players"
        );
      }
    }
    if (params[0].equals("off")) {
      if (params[1].toLowerCase().equals("y")) {
        ...
        PacketSendUtility.sendMessage(
          admin,
          "you are now Unavailable but can be Whisperable by players"
        );
      } else {
        PacketSendUtility.sendMessage(
          admin,
          "You are Offline without GM Tag, But people can Whisper you."
        );
      }
      if (admin.isGmMode()) {
        ...
        PacketSendUtility.sendMessage(
          admin,
          "You are unavailable to players now."
        );
      }
    }
