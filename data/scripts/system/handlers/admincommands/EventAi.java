    ...
    if (params[0].equals("rewardall_inzone")) {
      ...
      if (params[1].equals("gp")) {
        while (ita.hasNext()) {
          ...
          if (
            player.getWorldId() == admin.getWorldId() &&
            !(
              player.getName() == admin.getName() &&
              GeoService.getInstance().canSee(admin, player)
            )
          ) {
            AbyssPointsService.addGp(player, rewardAmount);
            PacketSendUtility.sendMessage(
              player,
              "You've rewarded " + rewardAmount + " GP from an Event!"
            );
            PacketSendUtility.sendMessage(
              admin,
              "Player : " + player.getName() + " has been rewarded!"
            );
          }
        }

        log.info(
          "[eventai-rewardall_inzone] GM : " +
          admin.getName() +
          " gave GP : " +
          rewardAmount +
          " in mapId '" +
        ...
        PacketSendUtility.sendMessage(
          admin,
          "Every player on the map has been sucessfully rewarded with " +
          rewardAmount +
          " GP !"
        );
        ...
        while (ita.hasNext()) {
          ...
          if (
            ...
            PacketSendUtility.sendMessage(
              player,
              "You've rewarded " + rewardAmount + " AP from an Event!"
            );
          }
        }
        log.info(
                  ...
                  if (MathUtil.isInRange(admin, player, range)) {
                    if (admin != player) {
                      ...
                      if (GP != 0) {
                        ...
                        PacketSendUtility.sendMessage(
                          player,
                          "You Received " + GP + " Glory Point(s) from Event!"
                        );
                      }
                      if (toll != 0) {
                        ...
                        PacketSendUtility.sendMessage(
                          player,
                          "You Received " + toll + " Toll Point(s) from Event!"
                        );
                      }
                      PacketSendUtility.sendWhiteMessage(
                        admin,
                        "Player: \uE020" +
                        player.getName() +
                      ...
                      PacketSendUtility.sendWhiteMessage(
                        ...
                        admin.getName() +
                        "'s Event, Have a Great Time Playing InsaneAion \uE020 "
                      );
                    }
                  }
                }
      ...
      boolean areyousure = admin
        .getResponseRequester()
        .putRequest(1300564, requestHim);
      if (areyousure) {
        PacketSendUtility.sendPacket(
          ...
          new SM_QUESTION_WINDOW(
            ...
            "x" +
            TS +
            " Tempering Solution and\n" +
            "x" +
            Omega +
            " Omega Enchantment Stone and\n" +
            "" +
            GP +
            " Glory Points and \n" +
            "" +
            toll +
            " Toll Points In Range Of \n" +
            "" +
            range +
            " Meters\n" +
      ...
      if (admin.getQueuedPlayers().isEmpty()) {
        PacketSendUtility.sendMessage(
          admin,
          "There is no one in the queued reward list!"
        );
        return;
      }
      ...
      log.info(
        "[eventai-movetomeall_inzone] GM : " +
        admin.getName() +
        " teleported everyone to him in mapId '" +
        admin.getWorldId() +
        "'"
      );
      PacketSendUtility.sendMessage(
        admin,
        "Every player in this Map as been gathered to Your location!"
      );
      ...
      log.info(
        "[eventai-resall_inzone] GM : " +
        admin.getName() +
        " resurrected everyone inzone in mapId '" +
        admin.getWorldId() +
        "'"
      );
      ...
      log.info(
        "[eventai-announce_inzone] GM : " +
        admin.getName() +
        " announced inzone saying [" +
        actual +
        "] in mapId '" +
        admin.getWorldId() +
        "'"
      ...
      log.info(
        "[eventai-announce_all] GM : " +
        admin.getName() +
        " announced all saying [" +
        actual +
        "] in mapId '" +
        admin.getWorldId() +
        "'"
      ...
      if (params[1].equalsIgnoreCase("all")) {
        ...
        while (ita.hasNext()) {
          ...
          if (
            player.getWorldId() == admin.getWorldId() &&
            player.getAccessLevel() < 1 &&
            GeoService.getInstance().canSee(admin, player)
          ) {
            SkillEngine
              .getInstance()
              .applyEffectDirectly(8256, admin, player, (10 * 1000));
          }
        }
        log.info(
          "[eventai-stop{all}] GM : " +
          admin.getName() +
          " paralyzed everyone in mapId '" +
          admin.getWorldId() +
          "'"
        );
      } else {
        ...
        SkillEngine
          .getInstance()
          .applyEffectDirectly(8256, admin, (Creature) target, (10 * 1000));
        SkillEngine
          .getInstance()
          .applyEffectDirectly(
            8256,
            admin,
            (Creature) targetsTarget,
            (10 * 1000)
          );
        log.info(
          "[eventai-stop{target}] GM : " +
          admin.getName() +
          " paralyzed both [" +
          target.getName() +
          "][" +
          targetsTarget.getName() +
          "] in mapId '" +
          admin.getWorldId() +
          "'"
        );
      ...
      while (ita.hasNext()) {
        ...
        if (
          admin.getWorldId() == player.getWorldId() &&
          player.getAccessLevel() < 1 &&
          GeoService.getInstance().canSee(admin, player)
        ) {
      ...
      log.info(
        "[eventai-returnall_inzone] GM : " +
        admin.getName() +
        " returned everyone in mapId '" +
        admin.getWorldId() +
        "'"
      );
      ...
      PacketSendUtility.sendMessage(
        ...
        "Player : " +
        player.getName() +
        " Ported to Jumping Instance StartPoint!"
      );

      log.info(
        "[eventai-port2jumping] GM : " +
        admin.getName() +
        " tried to port2jumping player [" +
        player.getName() +
        "] mapId '" +
        admin.getWorldId() +
        "'"
      );
    } else if (params[0].equalsIgnoreCase("test")) {} else {
      onFail(admin, null);
    }
  }
