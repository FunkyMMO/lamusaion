...
public class PlayerCommonData extends VisibleObjectTemplate {
  ...
  private Timestamp lastOnline = new Timestamp(
    Calendar.getInstance().getTime().getTime() - 20
  );
  private Timestamp lastStamp = new Timestamp(
    Calendar.getInstance().getTime().getTime() - 20
  );
  ...
  public Map<Integer, AtreianPassport> playerPassports = new HashMap<Integer, AtreianPassport>(
    1
  );
  ...
  public long getExpNeed() {
    ...
    return (
      DataManager.PLAYER_EXPERIENCE_TABLE.getStartExpForLevel(this.level + 1) -
      DataManager.PLAYER_EXPERIENCE_TABLE.getStartExpForLevel(this.level)
    );
  }
    ...
    if (this.getPlayer() != null) {
      PacketSendUtility.sendPacket(
        ...
        new SM_STATUPDATE_EXP(
          ...
          getExpRecoverable(),
          getExpNeed(),
          this.getCurrentReposteEnergy(),
          this.getMaxReposteEnergy(),
          this.getBerdinStar(),
      ...
      switch (rewardType) {
        ...
        case MONSTER_BOOK:
          if (npcNameId == 0) {
            PacketSendUtility.sendPacket(
              getPlayer(),
              SM_SYSTEM_MESSAGE.STR_GET_EXP2(reward)
            );
          } else {
            PacketSendUtility.sendPacket(
              getPlayer(),
              SM_SYSTEM_MESSAGE.STR_GET_EXP_DESC(
                new DescriptionId(npcNameId * 2 + 1),
                reward
              )
            );
            if (repose > 0) {
        ...
        case QUEST:
          if (npcNameId == 0) {
            ...
            PacketSendUtility.sendPacket(
              getPlayer(),
              SM_SYSTEM_MESSAGE.STR_GET_EXP_VITAL_MAKEUP_BONUS_DESC(
                new DescriptionId(npcNameId * 2 + 1),
                reward,
                repose,
                salvation
            ...
            PacketSendUtility.sendPacket(
              getPlayer(),
              SM_SYSTEM_MESSAGE.STR_GET_EXP_VITAL_BONUS_DESC(
                new DescriptionId(npcNameId * 2 + 1),
                reward,
                repose
              )
            ...
            PacketSendUtility.sendPacket(
              getPlayer(),
              SM_SYSTEM_MESSAGE.STR_GET_EXP_MAKEUP_BONUS_DESC(
                new DescriptionId(npcNameId * 2 + 1),
                reward,
                salvation
              )
        ...
        case PVP_KILL:
          if ((repose > 0) && (salvation > 0)) {
            PacketSendUtility.sendPacket(
              getPlayer(),
              SM_SYSTEM_MESSAGE.STR_GET_EXP_VITAL_MAKEUP_BONUS(
                name,
                reward,
                repose,
                salvation
            ...
            PacketSendUtility.sendPacket(
              getPlayer(),
              SM_SYSTEM_MESSAGE.STR_GET_EXP_MAKEUP_BONUS(
                name,
                reward,
                salvation
              )
            ...
            PacketSendUtility.sendPacket(
              getPlayer(),
              SM_SYSTEM_MESSAGE.STR_GET_EXP(name, reward)
            );
          }
          break;
        default:
  ...
  public void setExp(long exp, boolean ArchDaeva) {
    ...
    long maxExp = DataManager.PLAYER_EXPERIENCE_TABLE.getStartExpForLevel(
      maxLevel
    );
    if (getPlayerClass() != null && getPlayerClass().isStartingClass()) {
      ...
      if (this.getLevel() == 9 && this.getExp() >= 126069) {
        ...
        PacketSendUtility.sendPacket(
          this.getPlayer(),
          SM_SYSTEM_MESSAGE.STR_MSG_CAN_QUEST_DEVA
        );
      }
      ...
      if (!isCompleteQuest) {
        ...
        if (this.getExp() >= 2066885620) {
          ...
          PacketSendUtility.sendPacket(
            this.getPlayer(),
            SM_SYSTEM_MESSAGE.STR_MSG_CAN_QUEST_HIGHDEVA
          );
        }
      }
    }
    ...
    while (
      ...
      (
        up =
          exp >=
          DataManager.PLAYER_EXPERIENCE_TABLE.getStartExpForLevel(
            this.level + 1
          )
      ) ||
      (this.level - 1) >= 0 &&
    ...
    if (this.getPlayer() != null) {
      if (up && GSConfig.ENABLE_RATIO_LIMITATION) {
        if (
          this.level >= GSConfig.RATIO_MIN_REQUIRED_LEVEL &&
  ...
  public void setLevel(int level) {
    if (level <= DataManager.PLAYER_EXPERIENCE_TABLE.getMaxLevel()) {
      this.setExp(
          DataManager.PLAYER_EXPERIENCE_TABLE.getStartExpForLevel(level),
          false
        );
    }
  }
  ...
  public void setArchDaeva() {
    ...
    if (this.getLevel() < 66) {
      this.setExp(
          DataManager.PLAYER_EXPERIENCE_TABLE.getStartExpForLevel(66),
          true
        );
  ...
  public void setDp(int dp) {
    if (getPlayer() != null) {
      ...
      PacketSendUtility.broadcastPacket(
        getPlayer(),
        new SM_DP_INFO(playerObjId, this.dp),
        true
      );
      ...
      log.debug(
        "CHECKPOINT : getPlayer in PCD return null for setDP " +
        isOnline() +
        " " +
        getPosition()
      );
  ...
  public void checkBerdinStarPercent() {
    if ((this.getPlayer() != null) && (isReadyForBerdinStar())) {
      int percent = (int) (
        (float) berdinStar * 100.0 / (float) getMaxBerdinStar()
      );
      if ((!BerdinStarBoost) && (percent > 50)) {
        ...
        PacketSendUtility.sendPacket(
          this.getPlayer(),
          new SM_SYSTEM_MESSAGE(1403401, new Object[0])
        );
      }
    }
  }
  ...
  public void checkAbyssFavorPercent() {
    if ((this.getPlayer() != null) && (isReadyForAbyssFavor())) {
      int percent = (int) (
        (float) abyssFavor * 100.0 / (float) getMaxAbyssFavor()
      );
      if ((!AbyssFavorBoost) && (percent > 50)) {
        ...
        PacketSendUtility.sendPacket(
          this.getPlayer(),
          new SM_SYSTEM_MESSAGE(1404031, new Object[0])
        );
      }
    }
  }
