...
public final class PlayerSkillList implements SkillList<Player> {
  ...
  public PlayerSkillEntry[] getBasicSkills() {
    return basicSkills
      .values()
      .toArray(new PlayerSkillEntry[basicSkills.size()]);
  }

  public PlayerSkillEntry[] getStigmaSkills() {
    return stigmaSkills
      .values()
      .toArray(new PlayerSkillEntry[stigmaSkills.size()]);
  }

  public PlayerSkillEntry[] getLinkedSkills() {
    return linkedSkills
      .values()
      .toArray(new PlayerSkillEntry[linkedSkills.size()]);
  }
  ...
  public boolean isHaveHiddenStigma(Player player) {
    for (HiddenStigmasTemplate hst : DataManager.HIDDEN_STIGMA_DATA.getHiddenStigmasByClass()) {
      if (hst.getClassname().equals(player.getPlayerClass().name())) {
        for (PlayerSkillEntry pse : getStigmaSkills()) for (HiddenStigmasTemplate.HiddenStigmaTemplate oneStigma : hst.getHiddenStigmas()) if (
          oneStigma.getId().equals(pse.getSkillTemplate().getStack())
        ) return true;
        return false;
      }
    }
  ...
  public int getMaxAvailHiddenStigmaLvl() {
    ...
    for (PlayerSkillEntry playerSkillEntry : getStigmaSkills()) {
      if (playerSkillEntry.getSkillTemplate().getLvl() < lvl) lvl =
        playerSkillEntry.getSkillTemplate().getLvl();
    }
        ...
        for (PlayerSkillEntry pse : getStigmaSkills()) {
          for (HiddenStigmasTemplate.HiddenStigmaTemplate oneStigma : hst.getHiddenStigmas()) {
            if (oneStigma.getId().equals(pse.getSkillTemplate().getStack())) {
              ...
              if (!silent) PacketSendUtility.sendPacket(
                player,
                SM_SYSTEM_MESSAGE.STR_MSG_STIGMA_DELETE_HIDDEN_SKILL(
                  new DescriptionId(
                    DataManager.SKILL_DATA
                      .getSkillTemplate(pse.getSkillId())
                      .getNameId()
                  ),
                  skillLvl
                )
  ...
  private synchronized boolean addSkillAct(
    Player player,
    int skillId,
    int skillLevel,
    boolean isStigma,
    ...
    PlayerSkillEntry existingSkill = isStigma
      ? stigmaSkills.get(skillId)
      : basicSkills.get(skillId);
  ...
  public boolean addLinkedSkill(Player player, int skillId) {
    ...
    PacketSendUtility.sendPacket(
      player,
      SM_SYSTEM_MESSAGE.STR_MSG_STIGMA_GET_LINKED_SKILL(
        new DescriptionId(
          DataManager.SKILL_DATA
            .getSkillTemplate(linked.getSkillId())
            .getNameId()
        ),
        1
      )
  ...
  public void addHiddenStigmaSkill(Player player, int skillId, int skillLvl) {
    PlayerSkillEntry skill = new PlayerSkillEntry(
      skillId,
      false,
      true,
      skillLvl,
    ...
    PacketSendUtility.sendPacket(
      player,
      SM_SYSTEM_MESSAGE.STR_MSG_STIGMA_GET_HIDDEN_SKILL(
        new DescriptionId(
          DataManager.SKILL_DATA
            .getSkillTemplate(skill.getSkillId())
            .getNameId()
        ),
        skillLvl
      )
  ...
  public void addStigmaSkill(
    Player player,
    List<StigmaSkill> skills,
    boolean equipedByNpc
  ) {
    for (StigmaSkill sSkill : skills) {
      ...
      if (equipedByNpc) {
        PacketSendUtility.sendPacket(
          player,
          new SM_SKILL_LIST(skill, 1300401, false)
        );
      }
    }
  }
  ...
  public void addStigmaSkill(
    ...
    if (equipedByNpc) {
      PacketSendUtility.sendPacket(
        player,
        new SM_SKILL_LIST(skill, withMsg ? 1300401 : 0, false)
      );
    }
  }

  private synchronized boolean addSkill(
    Player player,
    int skillId,
    int skillLevel,
    boolean isStigma,
    ...
    PacketSendUtility.sendPacket(
      player,
      new SM_SKILL_LIST(player, player.getSkillList().getLinkedSkills())
    );
    return true;
  }
  ...
  public boolean addSkillXp(
    ...
    switch (skillEntry.getSkillId()) {
      ...
      case 40010:
        ...
        player
          .getRecipeList()
          .autoLearnRecipe(player, skillId, skillEntry.getSkillLevel());
    }
  ...
  public boolean isSkillPresent(int skillId) {
    return (
      basicSkills.containsKey(skillId) ||
      stigmaSkills.containsKey(skillId) ||
    ...
    switch (skillId) {
      ...
      case 30002:
        PacketSendUtility.sendPacket(
          player,
          new SM_SKILL_LIST(
            player.getSkillList().getSkillEntry(skillId),
            1330005,
            false
          )
        );
  ...
  public boolean addSkillWithoutSave(
    Player player,
    int skillId,
    int skillLevel
  ) {
