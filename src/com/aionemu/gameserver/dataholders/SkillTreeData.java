...
public class SkillTreeData {
   ...
   */
  public SkillLearnTemplate[] getTemplatesFor(
    ...
    List<SkillLearnTemplate> classRaceSpecificTemplates = templates.get(
      makeHash(playerClass.ordinal(), race.ordinal(), level)
    );
    List<SkillLearnTemplate> classSpecificTemplates = templates.get(
      makeHash(playerClass.ordinal(), Race.PC_ALL.ordinal(), level)
    );
    List<SkillLearnTemplate> generalTemplates = templates.get(
      makeHash(PlayerClass.ALL.ordinal(), Race.PC_ALL.ordinal(), level)
    );
  ...
  public void setStigmaTree() {
    for (SkillLearnTemplate skillLearnTemplate : skillTemplates) {
      String skillStack = DataManager.SKILL_DATA
        .getSkillTemplate(skillLearnTemplate.getSkillId())
        .getStack();
      int skillRealLvl = DataManager.SKILL_DATA
        .getSkillTemplate(skillLearnTemplate.getSkillId())
        .getLvl();
      ...
      } else addRaceList.add(skillLearnTemplate.getRace());
        ...
        if (stigmaTree.get(addRace) != null) {
          if (stigmaTree.get(addRace).get(skillStack) != null) {
            stigmaTree
              .get(addRace)
              .get(skillStack)
              .put(skillRealLvl, skillLearnTemplate.getSkillId());
          } else {
