...
public final class PlayerEnterWorldService {
  ...
  static {
    ...
    if (GSConfig.SERVER_MOTD_DISPLAYREV) {
      ...
      alBuffer +=
        ...
        String.format(
          " ",
          new Object[] { new Version(GameServer.class).getRevision() }
        ) +
        "\n";
    }
  ...
  public static final void startEnterWorld(
    final int objectId,
    final AionConnection client
  ) {
    PlayerAccountData playerAccData = client
      .getAccount()
      .getPlayerAccountData(objectId);
    Timestamp lastOnline = playerAccData.getPlayerCommonData().getLastOnline();
    if (
      lastOnline != null &&
      client.getAccount().getAccessLevel() < AdminConfig.GM_LEVEL
    ) {
      if (
        System.currentTimeMillis() -
        lastOnline.getTime() <
        (GSConfig.CHARACTER_REENTRY_TIME * 1000)
      ) {
        client.sendPacket(new SM_ENTER_WORLD_CHECK((byte) 6)); // 20 sec time
        return;
      }
    }
    CharacterBanInfo cbi = client
      .getAccount()
      .getPlayerAccountData(objectId)
      .getCharBanInfo();
    if (cbi != null) {
      if (cbi.getEnd() > System.currentTimeMillis() / 1000) {
        ...
        DAOManager
          .getDAO(PlayerPunishmentsDAO.class)
          .unpunishPlayer(objectId, PunishmentType.CHARBAN);
      }
    }
    if (
      SecurityConfig.PASSKEY_ENABLE &&
      !client.getAccount().getCharacterPasskey().isPass()
    ) {
  ...
  private static final void showPasskey(
    final int objectId,
    final AionConnection client
  ) {
    ...
    boolean isExistPasskey = DAOManager
      .getDAO(PlayerPasskeyDAO.class)
  ...
  private static final void validateAndEnterWorld(
    final int objectId,
    final AionConnection client
  ) {
  ...
  public static final void enterWorld(AionConnection client, int objectId) {
    ...
    if (player != null && client.setActivePlayer(player)) {
      ...
      if (playerAccData.getPlayerCommonData().getLastOnline() != null) {
        long lastOnline = playerAccData
          .getPlayerCommonData()
          .getLastOnline()
          .getTime();
        PlayerCommonData pcd = player.getCommonData();
        long secondsOffline =
          (System.currentTimeMillis() / 1000) - lastOnline / 1000;
        ...
        if (pcd.isReadyForReposteEnergy()) {
          ...
          if (secondsOffline > 14400) {
            ...
            if (
              player.getHouseOwnerId() / 10000 * 10000 == player.getWorldId()
            ) {
            ...
            pcd.addReposteEnergy(
              addResposeEnergy > maxRespose ? maxRespose : addResposeEnergy
            );
          }
        }
      ...
      client.sendPacket(
        new SM_SKILL_LIST(player, player.getSkillList().getBasicSkills())
      );
      for (PlayerSkillEntry stigmaSkill : player
        .getSkillList()
        .getStigmaSkills()) {
      ...
      client.sendPacket(
        new SM_TITLE_INFO(6, player.getCommonData().getBonusTitleId())
      );
      client.sendPacket(
        new SM_MOTION(player.getMotions().getMotions().values())
      );
      ...
      client.sendPacket(
        new SM_INSTANCE_INFO(player, false, player.getCurrentTeam())
      );
      ...
      if (player.isLegionMember()) {
        ...
        if (
          player.getLegionMember().isBrigadeGeneral() &&
          !player.getLegion().getJoinRequestMap().isEmpty()
        ) {
          client.sendPacket(
            new SM_LEGION_REQUEST_LIST(
              player.getLegion().getJoinRequestMap().values()
            )
          );
        }
      } else {
      ...
      client.sendPacket(
        new SM_EMOTION_LIST((byte) 0, player.getEmotions().getEmotions())
      );
      ...
      if (player.getAccessLevel() == 0) {
        for (int al : GmSpecialSkills
          .getAlType(GmSpecialSkills.AccessLevel5.getLevel())
          .getSkills()) {
      ...
      if (player.isGM()) {
        if (
          AdminConfig.INVULNERABLE_GM_CONNECTION ||
          ...
          if (AdminConfig.INVISIBLE_GM_CONNECTION) {
            player
              .getEffectController()
              .setAbnormal(AbnormalState.HIDE.getId());
        ...
        if (player.getAccessLevel() >= AdminConfig.GM_SPECIAL_SKILLS) {
          for (int al : GmSpecialSkills
            .getAlType(player.getAccessLevel())
            .getSkills()) {
      ...
      if (
        ...
        PacketSendUtility.sendBrightYellowMessageOnCenter(
          player,
          "Your account vip ! You get extra service!"
        );
      }
      // Ascension Boost.
      ...
      // Tip.
      PacketSendUtility.playerSendPacketTime(
        player,
        SM_SYSTEM_MESSAGE.STR_MSG_CHARGE_EXP_POINT,
        60000
      );
      // "Auto PowerShard ON"
      if (
        player.getEquipment().isPowerShardEquipped() &&
        CustomConfig.ENABLE_AUTO_POWERSHARD
      ) {
        PacketSendUtility.playerSendPacketTime(
          player,
          SM_SYSTEM_MESSAGE.STR_WEAPON_BOOST_BOOST_MODE_STARTED,
          7000
        );
        player.setState(CreatureState.POWERSHARD);
        PacketSendUtility.playerSendPacketTime(
          player,
          new SM_EMOTION(player, EmotionType.POWERSHARD_ON, 0, 0),
          7000
        );
      }
      ...
      client.sendPacket(
        new SM_RECIPE_LIST(player.getRecipeList().getRecipeList())
      );
      ...
      if (HTMLConfig.ENABLE_HTML_WELCOME) {
        HTMLService.showHTML(
          player,
          HTMLCache.getInstance().getHTML("welcome.xhtml")
        );
      }
      ...
      player.setPartnerId(
        DAOManager.getDAO(WeddingDAO.class).loadPartnerId(player)
      );
  ...
  public static final void abyssLightLogon(final Player player) {
    if (
      player.getAbyssRank().getRank().getId() ==
      AbyssRankEnum.SUPREME_COMMANDER.getId()
    ) {
      World
        .getInstance()
        .doOnAllPlayers(
          new Visitor<Player>() {
            ...
            public void visit(Player players) {
              ...
              PacketSendUtility.sendPacket(
                players,
                new SM_SYSTEM_MESSAGE(1403134, player.getName())
              );
            }
          }
        );
        ...
        .doOnAllPlayers(
          new Visitor<Player>() {
            ...
            public void visit(Player players) {
              ...
              PacketSendUtility.sendPacket(
                players,
                new SM_SYSTEM_MESSAGE(1403135, player.getName())
              );
            }
          }
        );
  ...
  private static void sendItemInfos(AionConnection client, Player player) {
    ...
    player
      .getInventory()
      .setLimit(StorageType.CUBE.getLimit() + (questExpands + npcExpands) * 9);
    player
      .getWarehouse()
      .setLimit(
        StorageType.REGULAR_WAREHOUSE.getLimit() + player.getWarehouseSize() * 8
      );
  ...
  private static void showPremiumAccountInfo(
  ...
  public static final void LoginServerInfo(Player player) {
    float pvpAttackRatio = player
      .getGameStats()
      .getStat(StatEnum.PVP_ATTACK_RATIO, 0)
      .getCurrent();
    float pvpDefenseRatio = player
      .getGameStats()
      .getStat(StatEnum.PVP_DEFEND_RATIO, 0)
      .getCurrent();
    float pvpAttackPhyscRatio = player
      .getGameStats()
      .getStat(StatEnum.PVP_ATTACK_RATIO_PHYSICAL, 0)
      .getCurrent();
    float pvpAttackMagicRatio = player
      .getGameStats()
      .getStat(StatEnum.PVP_ATTACK_RATIO_MAGICAL, 0)
      .getCurrent();
    float pvpDefensePhyscRatio = player
      .getGameStats()
      .getStat(StatEnum.PVP_DEFEND_RATIO_PHYSICAL, 0)
      .getCurrent();
    float pvpDefenseMagicRatio = player
      .getGameStats()
      .getStat(StatEnum.PVP_DEFEND_RATIO_MAGICAL, 0)
      .getCurrent();
    PacketSendUtility.sendPacket(
      player,
      new SM_SYSTEM_MESSAGE(1300564, "============================")
    );
    PacketSendUtility.sendPacket(
      player,
      new SM_SYSTEM_MESSAGE(
        1300564,
        "[color:Info：;0 1 0] Welcome to LAMUS Aion"
      )
    );
    PacketSendUtility.sendPacket(
      player,
    ...
    PacketSendUtility.sendPacket(
      ...
      new SM_SYSTEM_MESSAGE(
        1300564,
        "[color:Info：;0 1 0] Aion Account Information"
      )
    );
    PacketSendUtility.sendPacket(
      player,
      new SM_SYSTEM_MESSAGE(
        1300564,
        "[color:Info：;0 1 0][color:Toll Points：;0 1 0] " +
    ...
    PacketSendUtility.sendPacket(
      ...
      new SM_SYSTEM_MESSAGE(
        1300564,
        "[color:Info：;0 1 0][color:Luna Points：;0 1 0] " +
        player.getLunaAccount()
      )
    );
    PacketSendUtility.sendPacket(
      player,
    ...
    PacketSendUtility.sendPacket(
      ...
      new SM_SYSTEM_MESSAGE(
        1300564,
        "[color:Info：;0 1 0][color:Player Name：;0 1 0] " + player.getName()
      )
    );
    if (player.getAccessLevel() > 0) {
      PacketSendUtility.sendPacket(
        ...
        new SM_SYSTEM_MESSAGE(
          1300564,
          "[color:Info：;0 1 0][color:PvP Attack Ratio：;0 1 0] " +
          pvpAttackRatio *
          0.01f +
          "%"
        )
      ...
      PacketSendUtility.sendPacket(
        ...
        new SM_SYSTEM_MESSAGE(
          1300564,
          "[color:Info：;0 1 0][color:PvP Defense Ratio：;0 1 0] " +
          pvpDefenseRatio *
          0.01f +
          "%"
        )
      ...
      PacketSendUtility.sendPacket(
        ...
        new SM_SYSTEM_MESSAGE(
          1300564,
          "[color:Info：;0 1 0][color:PvP Physc Attack Ratio：;0 1 0] " +
          pvpAttackPhyscRatio *
          0.1f +
          "%"
        )
      ...
      PacketSendUtility.sendPacket(
        ...
        new SM_SYSTEM_MESSAGE(
          1300564,
          "[color:Info：;0 1 0][color:PvP Magic Attack Ratio：;0 1 0] " +
          pvpAttackMagicRatio *
          0.1f +
          "%"
        )
      ...
      PacketSendUtility.sendPacket(
        ...
        new SM_SYSTEM_MESSAGE(
          1300564,
          "[color:Info：;0 1 0][color:PvP Physc Defend Ratio：;0 1 0] " +
          pvpDefensePhyscRatio *
          0.1f +
          "%"
        )
      );
      PacketSendUtility.sendPacket(
        ...
        new SM_SYSTEM_MESSAGE(
          1300564,
          "[color:Info：;0 1 0][color:PvP Magic Defend Ratio：;0 1 0] " +
          pvpDefenseMagicRatio *
          0.1f +
          "%"
        )
      );
