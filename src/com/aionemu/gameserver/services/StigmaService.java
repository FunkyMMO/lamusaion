...
public class StigmaService {
  ...
  public static boolean extendAdvancedStigmaSlots(Player player) {
    int newAdvancedSlotSize =
      player.getCommonData().getAdvancedStigmaSlotSize() + 1;
   ...
   * @return
   */
  public static boolean notifyEquipAction(
    Player player,
    Item resultItem,
    long slot
  ) {
    if (resultItem.getItemTemplate().isStigma()) {
      if (ItemSlot.isRegularStigma(slot)) {
        ...
        if (
          getPossibleStigmaCount(player) <=
          player.getEquipment().getEquippedItemsRegularStigma().size()
        ) {
        ...
        if (
          getPossibleAdvencedStigmaCount(player) <=
          player.getEquipment().getEquippedItemsAdvancedStigma().size()
        ) {
          AuditLogger.info(
            player,
            "Possible client hack advanced stigma count big :O"
          );
          return false;
        }
      }
      ...
      if (
        !resultItem
          .getItemTemplate()
          .isClassSpecific(player.getCommonData().getPlayerClass())
      ) {
      ...
      if (!StigmaService.isPossibleEquippedStigma(player, resultItem)) {
        AuditLogger.info(
          player,
          "Player tried to get Multiple Stigma's from One Stigma Stone!"
        );
        return false;
      }
      ...
      if (stigmaInfo == null) {
        log.warn(
          "Stigma info missing for item: " +
          resultItem.getItemTemplate().getTemplateId()
        );
        return false;
      }

      int kinahCount = resultItem.getItemTemplate().getPrice();
      ...
      player
        .getSkillList()
        .addStigmaSkill(player, stigmaInfo.getSkills(), true);
      ...
      if (stigmaData.size() != 1) { // Dual Skill i.e Exhausting Wave
        for (Integer id : stigmaData) {
          stigmaUpperLvls =
            getStigmaInfoUpToLevel(player.getCommonData().getLevel(), id); // only 2 skill here '1:534 1:434
        ...
        // Single Skill
        stigmaUpperLvls =
          getStigmaInfoUpToLevel(
            player.getCommonData().getLevel(),
            stigmaData.get(0)
          ); // only 1 skill here '1:534'
        if (stigmaUpperLvls != null) {
          for (StigmaSkill stigma : stigmaUpperLvls) {
            player
              .getSkillList()
      ...
      List<Integer> sStigma = player
        .getEquipment()
        .getEquippedItemsAllStigmaIds();
   ...
   */
  public static List<StigmaSkill> getStigmaInfoUpToLevel(
    int playerLvl,
    int startingStigmaSkillid
  ) {
    List<StigmaSkill> stigmaList = new ArrayList<StigmaSkill>();
    SkillLearnTemplate[] temps = DataManager.SKILL_TREE_DATA.getTemplatesForSkill(
      startingStigmaSkillid
    );
    String skillName;
    ...
    for (SkillLearnTemplate skillTemp : temps) {
      ...
      for (int id = startingStigmaSkillid; id <= 5000; id++) {
        ...
        SkillLearnTemplate[] NextTemps = DataManager.SKILL_TREE_DATA.getTemplatesForSkill(
          id
        );
    ...
    if (resultItem.getItemTemplate().isStigma()) {
      ...
      if (itemId == 140000007 || itemId == 140000005) {
        if (equipment.hasDualWeaponEquipped(ItemSlot.LEFT_HAND)) {
          PacketSendUtility.sendPacket(
            player,
            SM_SYSTEM_MESSAGE.STR_STIGMA_CANNT_UNEQUIP_STONE_FIRST_UNEQUIP_CURRENT_EQUIPPED_ITEM
          );
          return false;
        }
      }
            ...
            if (rs.getSkillIds().contains(sSkill.getSkillId())) {
              PacketSendUtility.sendPacket(
                ...
                new SM_SYSTEM_MESSAGE(
                  1300410,
                  new DescriptionId(resultItem.getItemTemplate().getNameId()),
      ...
      for (StigmaSkill sSkill : stigmaInfo.getSkills()) {
        int nameId = DataManager.SKILL_DATA
          .getSkillTemplate(sSkill.getSkillId())
          .getNameId();
        PacketSendUtility.sendPacket(
          player,
          new SM_SYSTEM_MESSAGE(1300403, new DescriptionId(nameId))
        );
  ...
  public static void recheckHiddenStigma(Player player) {
    if (
      player.getLevel() >= 55 &&
      player.getEquipment().getEquippedItemsAllStigma().size() >= 6
    ) {
      for (Item stigma : player.getEquipment().getEquippedItemsAllStigma()) {
        if (stigma.getItemTemplate().isNoEnchant()) return;
      }
      int hiddenStigmaSkillId = DataManager.HIDDEN_STIGMA_DATA.getHiddenStigmaSkill(
        player
      );

      if (hiddenStigmaSkillId != 0) {
        if (
          !player.getSkillList().isHaveHiddenStigma(player) ||
          (
            player.getSkillList().isHaveHiddenStigma(player) &&
            player.getSkillList().getStigmaSkillEntry(hiddenStigmaSkillId) ==
            null
          )
        ) {
          player
            .getSkillList()
            .addHiddenStigmaSkill(player, hiddenStigmaSkillId, 1);
        }
      }
    }
  }
  ...
  public static void onPlayerLogin(Player player) {
    List<Item> equippedItems = player
      .getEquipment()
      .getEquippedItemsAllStigma();
    for (Item item : equippedItems) { // All Equipped Items are Stigmas
      if (item.getItemTemplate().isStigma()) {
        ...
        if (stigmaInfo == null) {
          log.warn(
            "Stigma info missing for item: " +
            item.getItemTemplate().getTemplateId()
          );
          return;
        }
        ...
        player
          .getSkillList()
          .addStigmaSkill(player, stigmaInfo.getSkills(), false);
  ...
  private static int getPossibleStigmaCount(Player player) {
    if (player == null || player.getLevel() < 20) return 0;
    ...
    if (player.getRace() == Race.ELYOS) {
      isCompleteQuest =
        player.isCompleteQuest(1929) ||
        (
          player.getQuestStateList().getQuestState(1929).getStatus() ==
          QuestStatus.START &&
          player
            .getQuestStateList()
            .getQuestState(1929)
      ...
      isCompleteQuest =
        player.isCompleteQuest(2900) ||
        (
          player.getQuestStateList().getQuestState(2900).getStatus() ==
          QuestStatus.START &&
          player
            .getQuestStateList()
            .getQuestState(2900)
    ...
    if (isCompleteQuest) {
      if (playerLevel < 30) return 2; else if (
        playerLevel < 40
      ) return 3; else if (playerLevel < 50) return 4; else if (
        playerLevel < 55
  ...
  private static int getPossibleAdvencedStigmaCount(Player player) {
    if (player == null || player.getLevel() < 45) return 0;
    ...
    if (player.getRace() == Race.ELYOS) {
      ...
      if (!player.isCompleteQuest(1929)) return 0;
      if (player.isCompleteQuest(11550)) return 6; else if (
        player.isCompleteQuest(30217) || player.isCompleteQuest(11276)
      ) return 5; else if (player.isCompleteQuest(11049)) return 4; else if (
        player.isCompleteQuest(3932)
      ) return 3; else if (player.isCompleteQuest(3931)) return 2; else if (
        player.isCompleteQuest(3930)
      ) return 1;
    } else {
      // Check whether Stigma Quests
      if (!player.isCompleteQuest(2900)) return 0;
      if (player.isCompleteQuest(21550)) return 6; else if (
        player.isCompleteQuest(30317) || player.isCompleteQuest(21278)
      ) return 5; else if (player.isCompleteQuest(21049)) return 4; else if (
        player.isCompleteQuest(4936)
      ) return 3; else if (player.isCompleteQuest(4935)) return 2; else if (
        player.isCompleteQuest(4934)
      ) return 1;
    }
    return 0;
  }
  ...
  private static boolean isPossibleEquippedStigma(Player player, Item item) {
    if (
      player == null || (item == null || !item.getItemTemplate().isStigma())
    ) return false;
    ...
    if (ItemSlot.isRegularStigma(itemSlotToEquip)) {
      ...
      if (stigmaCount > 0) {
        if (stigmaCount == 1) {
          if (itemSlotToEquip == ItemSlot.STIGMA1.getSlotIdMask()) return true;
        } else if (stigmaCount == 2) {
          if (
            itemSlotToEquip == ItemSlot.STIGMA1.getSlotIdMask() ||
    ...
    // Advenced Stigma
      ...
      if (advStigmaCount > 0) {
        if (advStigmaCount == 1) {
          if (
            itemSlotToEquip == ItemSlot.ADV_STIGMA1.getSlotIdMask()
          ) return true;
